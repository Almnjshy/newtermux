...
tile,
state.leftSnakeDirection,
state.leftSnakeRow,
state.leftSnakeCol,
state.rightSnakeDirection,
state.rightSnakeRow,
state.rightSnakeCol
)
...
...(end === 'left'
? {
leftSnakeDirection: position.direction,
leftSnakeRow: position.row,
leftSnakeCol: position.col,
}
: {
rightSnakeDirection: position.direction,
rightSnakeRow: position.row,
rightSnakeCol: position.col,
}),
...
lastMove: null,
isBlocked: false,
leftSnakeDirection: 'left',
leftSnakeRow: 0,
leftSnakeCol: 1,
rightSnakeDirection: 'right',
rightSnakeRow: 0,
rightSnakeCol: 1,
maxRow: 0,
minRow: 0,
...
export const canPlayerPlay = (
  player: Player,
  board: BoardTile[]
): boolean => {
  return player.hand.some(tile =>
    getValidEnds(tile, board).length > 0
  )
}
...
export const isGameBlocked = (
  ...
  return state.players.every(player =>
    !canPlayerPlay(player, state.board)
  )
}


export const getBlockedWinner = (
  state: GameState
): Player | null => {
  ...
  for (const player of state.players) {
    const score = player.hand.reduce(
      (sum, tile) => sum + tile.top + tile.bottom,
      0
    )
...
export const getAIMove = (
  state: GameState,
  playerIndex: number
): { tileIndex: number; end: TileEnd } | null => {
